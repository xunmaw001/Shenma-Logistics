    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="select" v-if="type!='info'"  label="运输状态" prop="yunshuzhuangtai">
          <el-select v-model="ruleForm.yunshuzhuangtai" placeholder="请选择运输状态">
            <el-option
        ...
        <div v-else>
          <el-form-item class="input" label="运输状态" prop="yunshuzhuangtai">
      ...
      <el-col :span="24">  
        <el-form-item class="upload" v-if="type!='info' && !ro.yunshuluxian" label="运输路线" prop="yunshuluxian">
          <file-upload
          tip="点击上传运输路线"
          action="file/upload"
          :limit="3"
          :multiple="true"
          :fileUrls="ruleForm.yunshuluxian?ruleForm.yunshuluxian:''"
          @change="yunshuluxianUploadChange"
          ></file-upload>
        </el-form-item>
        <div v-else>
          <el-form-item v-if="ruleForm.yunshuluxian" label="运输路线" prop="yunshuluxian">
            <img style="margin-right:20px;" v-bind:key="index" v-for="(item,index) in ruleForm.yunshuluxian.split(',')" :src="item" width="100" height="100">
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
      ...
      <el-col :span="12">
        ...
        <div v-else>
          ...
          <el-row>
            <el-col :span="24">
              <el-form-item v-if="type!='info'"  label="运输中" prop="yunshuzhong">
                <editor 
                    style="min-width: 200px; max-width: 600px;"
                    v-model="ruleForm.yunshuzhong" 
                    class="editor" 
                    action="file/upload">
                </editor>
              </el-form-item>
              <div v-else>
                <el-form-item v-if="ruleForm.yunshuzhong" label="运输中" prop="yunshuzhong">
                    <span v-html="ruleForm.yunshuzhong"></span>
                </el-form-item>
              </div>
            </el-col>
          </el-row>
...
export default {
  ...
  methods: {
    ...
    init(id,type) {
      ...
      if(this.type=='info'||this.type=='else'){
	    ...
	    this.ro.yunshuzhuangtai = true;
          ...
          if(o=='yunshuzhong'){
            this.ruleForm.yunshuzhong = obj[o];
	    this.ro.yunshuzhong = true;
            continue;
          }
          if(o=='yunshuluxian'){
            this.ruleForm.yunshuluxian = obj[o];
	    this.ro.yunshuluxian = true;
            continue;
          }
          if(o=='zhanghao'){
      ...
      this.$http({
        ...
        if (data && data.code === 0) {
          ...
          this.$message.error(data.msg);
            ...
            this.yunshuzhuangtaiOptions = "待发货,未运输,等待揽收,运输中,派件中".split(',')
            this.$http({
    ...
    zhanghaoChange () {
      this.$http({
        ...
        if (data && data.code === 0) {
              ...
              if(data.data.shouhuodizhi){
                this.ruleForm.shouhuodizhi = data.data.shouhuodizhi
              }
        } else {
          this.$message.error(data.msg);
        }
	...
	this.ruleForm.yunshuzhong = this.ruleForm.yunshuzhong.replace(reg,'../../../ssmbgp63/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
    ...
    onSubmit() {
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `wuliuzhuizong/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.wuliuzhuizongCrossAddOrUpdateFlag = false;
                  this.parent.search();
    ...
    yunshuluxianUploadChange(fileUrls) {
	this.ruleForm.yunshuluxian = fileUrls;
			this.addEditUploadStyleChange()
    },
	addEditStyleChange() {
	  this.$nextTick(()=>{
